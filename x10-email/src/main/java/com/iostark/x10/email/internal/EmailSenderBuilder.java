...
public class EmailSenderBuilder {
   ...
   */
  public EmailSenderBuilder setProviderType(
      final EmailProviderType type, final Map<String, Object> params) {
    providerType = type;
    return this;
  }
  ...
  private static class BaseEmailService implements EmailSender {
    ...
    private static final SendListener SEND_LISTENER_NOP =
        new SendListener() {
          @Override
  ...
  private class DefaultAppEmailService extends BaseEmailService {
    ...
    public void send(final Email email, final SendListener listener) {
      final String[] tos =
          email.getToRecipients().toArray(new String[email.getToRecipients().size()]);
