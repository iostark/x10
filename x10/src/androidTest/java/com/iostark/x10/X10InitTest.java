...
public class X10InitTest extends ActivityInstrumentedTest<X10TestActivity> {
    ...
    public void testFOSSLicence() throws Exception {
        ...
        X10Init data = new X10Init()
                .useContext(activity.getApplicationContext())
                .useFOSSLicense();

        X10.init(data, new X10InitializationCallback() {
            @Override
            public void onFailure(Exception exception) {
                asyncTest.setAssertion(new AsyncAssert.Assertable() {
                    @Override
