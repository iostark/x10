...
public class Permission {
  ...
  private static final String SHOWN_RATIONALE_AFTER_PERMISSION_REFUSAL_KEY =
      "show_rationale_after_permission_refusal";
   ...
   *
   * @param permissionName The permission to request. Eg: {@link
   *     android.Manifest.permission#ACCESS_FINE_LOCATION}
   * @param requestHandler Permission handler receive permission results. You can use {@link
   *     BasePermissionRequestHandler}
  ...
  public void requestAppPermission(
      ...
      @Nullable final PermissionRequestHandler requestHandler,
      @Nullable final PermissionRationaleHandler rationaleHandler) {
   ...
   * @param requestHandler Object to receive permission results. You can use {@link
   *     BasePermissionRequestHandler}
   * @param rationaleHandler Permission rationale handler to receive decision from the user.
   * @since V0_5_0
   */
  ...
  public void requestAppPermission(
    ...
    LOGGER.v(
        "[%d] Requesting app permission %s with request code %d",
        requestCode, permissionName, requestCode);

    // 1. Verify if permission is already granted
    final int permissionResult = checkSelfPermission(context, permissionName);

    LOGGER.d(
        "[%d] Currently app permission %s is %s",
        requestedCode, permissionName, PermissionAccess.from(permissionResult));
    ...
    switch (permissionResult) {
      ...
      case PackageManager.PERMISSION_DENIED:
        if (this.rationaleHandler.showRationaleBeforeRequestingPermission()) {
          // 2a. Show Rationale
          final PermissionRationaleResult rationaleResult =
              new PermissionRationaleResult() {
                @Override
          ...
          LOGGER.d(
              "[%d] Show permission rationale before requesting permission for %s",
              requestedCode, permissionName);
  ...
  public void onRequestPermissionsResult(
    ...
    for (int i = 0; i < permissions.length; i++) {
      ...
      if (permissionResult == PackageManager.PERMISSION_DENIED
          && rationaleHandler.showRationaleAfterRefusingPermission()
          && !shownRationaleAfterPermissionRefused) {
        ...
        LOGGER.d(
            "[%d] Show permission rationale after requesting permission for %s",
            requestedCode, permissionName);
        requestHandler.onShowPermissionRationale(permissionName, rationaleResult);
        return;
      } else {
        ...
        LOGGER.d(
            "[%d] Final permission request result for %s: %s",
            requestedCode, permissionName, PermissionAccess.from(permissionResult));
  ...
  /**
   * To be called when {@link Activity#onSaveInstanceState(Bundle)} or {@link
   * Activity#onSaveInstanceState(Bundle, PersistableBundle)} is called.
   *
  ...
  public void onSaveInstanceState(final Bundle outState) {
    final X10Bundle x10Bundle =
        new X10Bundle(outState, ModuleConfig.MODULE_NAME, Permission.class.getSimpleName());
    x10Bundle.putInt(REQUEST_CODE_KEY, requestedCode);
    x10Bundle.putBoolean(
        SHOWN_RATIONALE_AFTER_PERMISSION_REFUSAL_KEY, shownRationaleAfterPermissionRefused);
  }

  /**
   * To be called when {@link Activity#onRestoreInstanceState(Bundle)} or {@link
   * Activity#onRestoreInstanceState(Bundle, PersistableBundle)} is called.
   *
  ...
  public void onRestoreInstanceState(final Bundle savedInstanceState) {
    final X10Bundle x10Bundle =
        new X10Bundle(
            savedInstanceState, ModuleConfig.MODULE_NAME, Permission.class.getSimpleName());
    requestedCode = x10Bundle.getInt(REQUEST_CODE_KEY, REQUEST_CODE);
    shownRationaleAfterPermissionRefused =
        x10Bundle.getBoolean(SHOWN_RATIONALE_AFTER_PERMISSION_REFUSAL_KEY, false);
  }

  private void requestPermission2(
      final Activity activity, final String permissionName, final int requestCode) {
    requestPermission2(activity, new String[] {permissionName}, requestCode);
  }

  private void requestPermission2(
      final Activity activity, final String[] permissionNames, final int requestCode) {
