...
public class PermissionTest extends ActivityInstrumentedTest<PermissionActivity> {
  ...
  public void testPermission1() throws UiObjectNotFoundException {
    PermissionRequestHandler requestHandler =
        new PermissionRequestHandler() {
          ...
          public void onShowPermissionRationale(
              String permissionName, @NonNull PermissionRationaleResult rationaleResult) {
            fail();
          }

          @Override
          public void onNeverAskAgainPermission(
              String permissionName, int currentPermissionStatus) {
            fail();
          }
    ...
    permission.requestAppPermission(
        Manifest.permission.ACCESS_FINE_LOCATION, 100, requestHandler, rationaleHandler);
  ...
  public void testPermission2() throws UiObjectNotFoundException {
    ...
    PermissionRequestHandler requestHandler =
        new PermissionRequestHandler() {
          ...
          public void onPermissionRequestResult(String permissionName, int permissionStatus) {
            result.put(
                GOT_PERMISSION_1_RESULT, PackageManager.PERMISSION_DENIED == permissionStatus);
        ...
        new PermissionRequestHandler() {
          ...
          public void onPermissionRequestResult(String permissionName, int permissionStatus) {
            if (result.get(EXPECT_PERMISSION_2)) {
              result.put(
                  GOT_PERMISISON_2_RESULT, PackageManager.PERMISSION_DENIED == permissionStatus);
    ...
    PermissionRequestHandler requestHandler =
        new PermissionRequestHandler() {
          ...
          public void onPermissionRequestResult(String permissionName, int permissionStatus) {
            if (result.get(EXPECT_PMR_2)) {
              result.put(
                  GOT_PERMISSION_2_RESULT, PackageManager.PERMISSION_DENIED == permissionStatus);
  ...
  private UiObject clickPermissionDialog(String buttonText) throws UiObjectNotFoundException {
    if (Build.VERSION.SDK_INT >= 23) {
      UiObject uiObject =
          device.findObject(
              new UiSelector().clickable(true).enabled(true).textContains(buttonText));
      if (uiObject.exists()) {
        uiObject.click();
      }
      return uiObject;
  ...
  private void waitUntilPermissionDialogIsDisplayed() {
    ...
    if (Build.VERSION.SDK_INT >= 23) {
      ...
      UiObject permissionDialogProof2 =
          device.findObject(new UiSelector().clickable(true).enabled(true).textContains("ALLOW"));
      UiObject permissionDialogProof3 =
          device.findObject(new UiSelector().clickable(true).enabled(true).textContains("DENY"));

      boolean isPermissionDialogDisplayed = false;

      while (!isPermissionDialogDisplayed) {
        isPermissionDialogDisplayed = /*permissionDialogProof1.exists() && */
            permissionDialogProof2.exists() && permissionDialogProof3.exists();
      }
    } else {
      // Not supported
    }
  ...
  private static void resetPermission(String packageName, String permissionName)
